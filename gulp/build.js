...
gulp.task('oauth', function () {
  return gulp.src(path.join(conf.paths.oauth, '/*'))
  .pipe($.filter('*.{html,js.sample,js}'))
  .pipe($.flatten())
  .pipe(gulp.dest(path.join(conf.paths.dist, '/oauth/')));
...
gulp.task('build', ['html', 'fonts', 'other', 'oauth']);
